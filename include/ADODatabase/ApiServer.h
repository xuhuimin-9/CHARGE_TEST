...
public:
	...
	// 监听所有本地IP的指定端口
	bool InitialAPI();
	//  TODO 暂未实现 - 监听RabbitMQ指定端口 [8/10/2021 Meixu]
	bool InitialRabbitMQ(std::string routing_key);

	// send 默认走 post
	inline bool send(std::string path, ContentType content_type, QJsonObject body, bool operation(QJsonObject) = nullptr) {
		post(path, content_type, body, operation);
	};

	bool get(std::string path, ContentType content_type, bool operation(std::string, QJsonObject) = nullptr);

	bool post(std::string path, ContentType content_type, QJsonObject body, bool operation(QJsonObject) = nullptr);
...
private:
	...
	QNetworkRequest send(std::string path, ContentType content_type, QJsonObject body, QNetworkRequest& head, QByteArray& bytebody);

	QNetworkRequest send(std::string path, ContentType content_type);
...
public:
	...
	void generateReturnBody(bool valid,std::string name,int code = DefaultErrorCode, const char* msg = VNAME(DefaultErrorCode), QJsonObject *object = new QJsonObject());
	...
	static QJsonArray ReadArray(QJsonObject object, QString key);
