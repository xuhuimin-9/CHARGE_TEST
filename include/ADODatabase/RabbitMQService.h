...
public:
	...
	// 复杂逻辑
	// 接口1:申请进入水箱
	// 0是发送中,1是返回滚筒线没有货物,2是滚筒线有货物
	int ApplyUnit(std::string UnitId, std::string TaskId);
	// 接口2:开始取料,并询问是否需要打断
	// 2.1:同步
...
private:
	...
	//连接
	CRabbitMqClient* mqClient_ = nullptr;
	CExchange* exchange_;
	...
	// 定义操作函数
	std::map<std::string, bool(RabbitMQService::*)(QJsonObject)> launcher;
	bool InsertOrder(QJsonObject object);
	...
	std::map<std::string, int> all_apply_unit_status_count_;
	...
	// 库位名映射
	std::map<std::string, std::string> storage_type_;
};
