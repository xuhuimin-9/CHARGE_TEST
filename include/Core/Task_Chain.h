...
public:
	...
	Task_Chain(WCS_Database * WCS_DB = nullptr);
	...
	std::string getTrayGet(bool release);

	std::string getExitTrayPut();
	...
	std::string getBufferConfirm();

	std::string getBufferParkingConfirm();

	std::string getBufferRelease();

	std::string getBufferParkingRelease();
	...
	void setExitTrayPut(std::string exit_tray_put);
	...
	void setBufferConfirm(std::string buffer_confirm);

	void setBufferParkingConfirm(std::string buffer_parking_confirm);

	void setBufferRelease(std::string buffer_release);

	void setBufferParkingRelease(std::string buffer_parking_release);
	...
	bool fork_status = true;//满轮区域被锁后，是否需要原地降叉指令

	bool dispatch_status = false;// 标志位，本次循环中，子任务是否满足下发条件
protected:

	TASK_CHAIN_STATUS createInStorageTaskChain();

	TASK_CHAIN_STATUS createOutStorageTaskChain();
...
private:
	...
	bool Charge_Station_Task(int agv_id, int station_id, std::string mode);
	...
	bool TRAY_GET_RELEASE_FLAG_ = true;// 是否释放过该点

	std::string TRAY_PUT_;

	std::string EXIT_TRAY_PUT_;

	std::string GOODS_GET_;

	std::string GOODS_PUT_;

	std::string BUFFER_CONFIRM_;

	std::string BUFFER_PARKING_CONFIRM_;

    std::string	BUFFER_RELEASE_;

	std::string	BUFFER_PARKING_RELEASE_;
