...
void AGV_Monitor::iniData()
{
	...
	headers << tr("ID")<<tr("Type")<<tr("Status")<<tr("Error Code")<<tr("Resource Status")<<tr("Network")<<tr("Dispatched")<<tr("Battery")<<tr("In Station") << tr("Msg");
...
void AGV_Monitor::updateProgressValue()
{
	...
	{  
		...
		rowset.push_back(codec->toUnicode(current_agv->msg_.c_str()));
		data.append(rowset);
...
void AGV_Monitor::popMenu(const QPoint&)
{
	...
	if(list.count()>1)
	{
		...
		connect(&setIdle, SIGNAL(triggered()), this, SLOT(idleAGV()));
...
void AGV_Monitor::Unlock_AGV()
{
	...
	if(agv_id != -1)
	{
		...
		{
		case 0:
			AGV_MANAGE.resetAgvsLineLength(agv_id);
			AGV_MANAGE.setManualUnlock(agv_id);
			AGV_MANAGE.setStopCharging(agv_id);
			AGV_MANAGE.setAgvPauseContinue(agv_id, "force continue");
