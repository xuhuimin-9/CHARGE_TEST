...
void DOD_Monitor::iniData()
{
	...
	headers << tr("Order ID")<<tr("Start")<<tr("Target")<<tr("Priority")<<tr("Status")<<tr("Mode")<<tr("Type")<<tr("Enter Date") << tr("MSG");
...
void DOD_Monitor::translatorUI()
{
	...
	headers << tr("ORDER_ID") << "START" <<"TARGETED" << "PRIORITY" << "STATUS"<< "MODE" << "TYPE" << "ENTER DATE" << tr("MSG");
	m_model->setHorizontalHeader(headers);
...
void DOD_Monitor::updateProgressValue()
{
	try
	{
		...
		for (; it != DOD_List.end(); ++it)
		{
			...
			//rowset.push_back(QString::fromStdString(current_order.Msg_));
