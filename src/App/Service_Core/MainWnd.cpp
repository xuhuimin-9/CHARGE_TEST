...
MainWindow::MainWindow() 
{
	...
	CONFIRM_MANAGE.setConfirmStatusNotifier(this);
	PARKING_MANAGE.setStationStatusNotifier(this);
	//STORAGE_MANAGE.Set_MainWindow_Delegate(this);
	
	// API传输数据
	//API_CLIENT.setApiClientNotifier(this);
...
void MainWindow::CreateMenu()
{
	...
	OrderActionMenu = menuBar()->addMenu(tr("Order"));
	OrderActionMenu->addAction(order_start_);
	OrderActionMenu->addAction(order_stop_);
}
...
void MainWindow::CreateActions()
{
	...
	order_start_ = new QAction(tr("Start Dispatch"), this);
	order_start_->setStatusTip(tr("Start Dispatch"));
	connect(order_start_, SIGNAL(triggered()), this, SLOT(start_order()));

	order_stop_ = new QAction(tr("Stop Dispatch"), this);
	order_stop_->setStatusTip(tr("Stop Dispatch"));
	connect(order_stop_, SIGNAL(triggered()), this, SLOT(stop_order()));
...
void MainWindow::start_system()
{
	...
	start_run_->setEnabled(false);
	//StartMenu->setEnabled(false);
	start_run();
}

void MainWindow::stop_order()
{
	EVENT_HANDLER.start_run = false;
}

void MainWindow::start_order()
{
	EVENT_HANDLER.start_run = true;
}

void MainWindow::timerEvent( QTimerEvent *event )
{
	//轮询 - 初始化停车点与确认点信息
	//PARKING_MANAGE.checkStationStatus();
	cTimerDiff dt;
	F64 Event_Time=0;

	dt.Begin();
	PLC_MANAGE.UpdateStorage();
	STORAGE_MANAGE.checkChange();
	CONFIRM_MANAGE.checkStationStatus();
	PARKING_MANAGE.checkStationStatus();
	CONFIG_MANAGE.reReadConfig();
	...
	if (Event_Time > 1500)
	{
		std::cout << "API_CLIENT.updateAllStorageView: " << Event_Time << std::endl;
	}

	dt.Begin();
	AGV_MANAGE.Update_AGV_Data_From_ACS_Event();
	AREA_MANAGE.areaControl();
...
{
	/*//RABBITMQ_SERVICE.ApplyGetGoods("123", "123");
	//return;
	...
	else*/
	{
		...
		ORDER_MANAGE.Insert_Order(GUID.toStdString(), START, TARGET, PRIORITY, STATUS, MODE, TYPE);
	}
}
...
void MainWindow::createChangeStorageStatusDialog()
{
	...
	QPushButton *change_to_empty = new QPushButton(tr("Empty"), this);
	QPushButton *change_to_full = new QPushButton(tr("Full"), this);
...
void MainWindow::loadSendOrderInterface()
{
	...
	// 获取所有停车点
	std::map<std::string, Confirm>  allConfirmStation = CONFIRM_MANAGE.getAllConfirmStation();
	std::vector<ModelStationParking*>  allParkingStation = PARKING_MANAGE.getAllParkingStation();
	std::map<std::string, ModelEquipment*> allEquipment = PARKING_MANAGE.getAllEquipment();
	...
	{ // Confirm Station
		...
		{
			Confirm station = iter.second;
			SendOrderConfirmLabel* comfirm = new SendOrderConfirmLabel(station.Storage_Name_.c_str(), this);
			comfirm->set_background_color_of_label(station.Storage_Status_);
			...
			confirmLabelList[station.Storage_Name_.c_str()] = comfirm;
		}
	}
	...
	// 从 数据库 读取 modeName
	std::vector<std::string>  allModeName = MODE_MANAGE.getAllTaskName();
...
void MainWindow::connectMylabelSlots()
{
	...
	for each (std::pair<std::string , std::map<std::string , MyLabel*>> var in mylabel_map_list)
	{
		for each (std::pair<std::string , MyLabel*> var1 in var.second)
		{
			...
			connect(var1.second, SIGNAL(sendChangeTrayType(std::string, std::string)), this, SLOT(changeStorageTrayType(std::string, std::string)));
		}
	}
}
...
{
	if (storageLabelList[storage.Storage_Name_])
	{
...
void MainWindow::OnConfirmStatusChanged(Confirm station)
{
	if (confirmLabelList.find(station.Storage_Name_) != confirmLabelList.end())
	{
		emit signal_change_mylabel_status(confirmLabelList[station.Storage_Name_], station.Storage_Status_);
	}
}
...
{
	CONFIG_MANAGE.readConfig("LANGUAGE", &language_);
}
...
void MainWindow::slot_change_mylabel_status(MyLabel* label , std::string storage_status, std::string resource_status_, bool flag1, bool flag2)
{
	if (label != nullptr)
	{
		if (resource_status_ != "IDLE" && !resource_status_.empty())
		{
...
void MainWindow::changeStorageTrayType(std::string storage_name, std::string type)
{
	QMessageBox msgBox;
	QString msg;
	if (STORAGE_MANAGE.changeStorageTrayType(storage_name, type))
	{
		// 弹窗提示更新成功
		msg = tr(" Success");
	}
	...
	msgBox.setText(storage_name.c_str() + tr(" change to ") + type.c_str() + msg);
	log_info(storage_name + " change to " + type + msg.toStdString());
	msgBox.setStandardButtons(QMessageBox::Ok);
	msgBox.exec();
}
...
void MainWindow::slot_rabbitmq_insert_order(std::string taskId, int taskType, int taskStatus, std::string unitId, std::string palletNo)
{
	if (taskStatus == 0)
	{
		...
		log_info_color(green, "ORDER_MANAGE.InsertOrder:" + taskId);
		ORDER_MANAGE.Insert_Order(taskId, unitId, "", PRIORITY, "NEW", MODE, "CARRY", agv_id, palletNo);
	}
	else if (taskStatus == 3)
	{
		if (ORDER_MANAGE.Revoke_Order(taskId))
		{
			// 取消工单成功
			log_info_color(green, "ORDER_MANAGE.Revoke_Order Success:" + taskId);
		}
		else
		{
			log_info_color(red, "ORDER_MANAGE.Revoke_Order Fail:" + taskId + ", unitId:" + unitId + ", MODE=" + std::to_string(taskStatus));
		}
	}
	return;
}