...
bool ApiServer::InitialRabbitMQ(std::string routing_key)
{
	bool result = false;
	return result;
}
...
void ApiServer::getNewConnection(QTcpServer* server) {
	...
	connect(cli,
		...
		[=]() {
		killTimer(timer);
		log_error("ApiServer: SocketError: " + cli->errorString().toStdString());
	}
	); /* 错误反馈 */
	connect(cli,
...
void ApiServer::timerEvent(QTimerEvent *event) {
	...
	point_msg->generateReturnBody(false,"AGV", TimeoutError, VNAME(TimeoutError));
...
bool ApiServer::launchOperation(ApiMsg* msg) {
	...
	else
	{
		// result = (launcher["-1"])(msg);
		// 没有默认的错误处理函数
		log_error("ApiServer::launchOperation: Get Unknown Operation but No Default ");
	}
	return result;
}
...
QNetworkRequest ApiServer::send(std::string path, ContentType content_type, QJsonObject body, QNetworkRequest& head, QByteArray& bytebody) {
...
// 建立连接
	...
	bytebody = ApiMsg::TranscodeToByte(body, content_type);
...
// get 请求组件报文头
QNetworkRequest ApiServer::send(std::string path, ContentType content_type) {
	// QNetworkRequest head = ApiMsg::generateApplyHead(applicationjson, "", 625, "/");
	// 建立连接
	if (accessManager == nullptr)
	{
...
// Description:
//************************************
bool ApiServer::post(std::string path, ContentType content_type, QJsonObject body, bool operation(QJsonObject) /*= nullptr*/)
{
	QNetworkRequest head;
	QByteArray bytebody;
	send(path, content_type, body, head, bytebody);
	// 发送
...
bool ApiServer::get(std::string path, ContentType content_type, bool operation(std::string, QJsonObject) /*= nullptr*/)
{
	QNetworkRequest head;
	head = send(path, content_type);
	// 发送
	QNetworkReply *network_reply = accessManager->get(head);
	...
	connect(network_reply, &QNetworkReply::readyRead,
		...
		QJsonObject data;
		bool analysis_result = ApiMsg::AnalysisDatatoJson(response_body, content_type, &data);
		if (operation && (operation)(path, data))
		{
			// 操作成功
		}
		else
...
bool ApiServer::send(std::string path, ContentType content_type, QJsonObject body, bool operation(QJsonObject, QJsonObject))
{
	...
	//超时处理
	QReplyTimeout *pTimeout = new QReplyTimeout(network_reply, 1000);
	...
	connect(network_reply, &QNetworkReply::readyRead,
		...
		{
			std::stringstream ss;
			ss << "recv data but operation fail!";
			log_info_color(log_color::purple, ss.str().c_str());
			// 发送消息成功，获取响应消息成功，但对响应消息的处理失败
			analysis_result;
		}
...
bool ApiMsg::analysisBodyFromJson() {
	...
	{ // JSON读取没有出错
		if (doucment.isObject())
		{
			...
			operationName = ReadString(bodyContent, "operatorName");
...
QJsonArray ApiMsg::ReadArray(QJsonObject object, QString key) {
	QJsonArray value;
	if (object.contains(key))
	{  // 包含指定的 key
		QJsonValue json_value;
...
// 否则,由指定错误填充
//************************************
void ApiMsg::generateReturnBody(bool valid, std::string name ,int code /*= DefaultErrorCode*/, const char* msg /*= VNAME(DefaultErrorCode)*/, QJsonObject *object /*= new QJsonObject()*/) {
	...
	object->insert("operatorName", QJsonValue(QString::fromStdString(name)));
	if (valid)
	{
