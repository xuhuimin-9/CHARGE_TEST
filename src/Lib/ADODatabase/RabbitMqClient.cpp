...
int32_t CRabbitMqClient::Connect(const std::string virtualHost /*= std::string("/")*/, std::string &ErrorReturn /*= ""*/)
{
	...
	if (m_connectState == nullptr)
	{
		ErrorReturn = "unknown error, connection failed";
		return -1;
	}
...
int32_t CRabbitMqClient::ConsumerMessage(std::string &routing_key, std::string &message, uint64_t &delivery_tag, const std::string &queue_name_, timeval timeout/* = { 5, 0 }*/, std::string &ErrorReturn/* = std::string("")*/)
{
	//step3
	...
	routing_key = std::string((char *)envelope.routing_key.bytes, (char *)envelope.routing_key.bytes + envelope.routing_key.len);
	//std::string consumer_tag = std::string((char *)envelope.consumer_tag.bytes, (char *)envelope.consumer_tag.bytes + envelope.consumer_tag.len);
	//std::string exchange = std::string((char *)envelope.exchange.bytes, (char *)envelope.exchange.bytes + envelope.exchange.len);
