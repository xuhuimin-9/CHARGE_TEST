...
#define  PORT 4001
#define  IP_ADDRESS "192.168.0.101" /* 本地ip设置成了192.168.0.101 已映射到navicat模块*/
...
* 结果：真或假
*/
boolean charge_station::set_max_current(int value)
{
	max_current_ = value;
	log_info("set max current : %d", max_current_);
	return true;
}
...
int charge_station::get_max_current()
{
	log_info("get max current : %d", max_current_);
	return max_current_;
}
...
* 结果：真或假
*/
boolean charge_station::set_charge_current(int value)
{
	charge_current_ = value;
	log_info("set charge current : %d", charge_current_);
	return true;
}
...
boolean charge_station::send_packet(void* p_packet, int size)
{
	...
	while (1)
	{
		...
		if (num == SOCKET_ERROR)
		{
			log_info("error happened in send_packet：%d", WSAGetLastError());
			socket_state = FALSE;
			return FALSE;
		}
	...
	while (!stop)
	{
		...
		if (TRUE == read_msg(p_charge->get_queue(), &p_void, &len, &type))
		{
			...
			case START_CHARGE_MSG_MACRO:
				...
				result = p_charge->run_charge(p_charge->get_charge_current());//配置充电电流为当前温度允许电流
				printf("result = %s\n", result == true ? "ok" : "error");
				Sleep(500);
			...
			case ARM_IN_MSG_MACRO:
				...
				log_info("take back arm");
				Sleep(500);
				result = p_charge->get_control_state(&error);
				log_info("result = %s", result == true ? "ok" : "error");
