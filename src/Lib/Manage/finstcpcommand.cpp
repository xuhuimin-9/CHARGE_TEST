...
#ifndef _SILENCE_CXX17_ALLOCATOR_VOID_DEPRECATION_W 
#define _SILENCE_CXX17_ALLOCATOR_VOID_DEPRECATION_W 
#endif
...
*          num_bits: 需要读取的位数
*/
bool finsTcpCommand::fins_readbit(const char* start, bool* resdata, std::size_t num_bits, std::string & msg /*= std::string("")*/)
{
	...
	msg += cComm::ByteToHexString(send_buf, 34);
	delete[] send_buf;
	send_buf = 0;
	...
	//std::string message  = (char*)rec_buf;
	msg += "\n";
	msg += cComm::ByteToHexString(rec_buf, rec_len);
	msg += "\nrec_len = " + std::to_string(rec_len) + "; len = " + std::to_string(len);
	...
	{
		no_error = true;
		for (unsigned int a = 0; a < chunk_length; a++)
		{
